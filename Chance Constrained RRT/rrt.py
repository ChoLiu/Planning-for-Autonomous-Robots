...
def augmentMap(mymap, res=0.2, clear=3):
...
def testReachability(mymap, vertex, direction, stepSize):
    newNode = vertex
    # print("vertex, ", vertex)
    # print("dir, ", direction)

    for step in range(1, int(stepSize)):
        ...
        if array_equal(mymap[tmp], [0, 0, 0]) or array_equal(mymap[tmp], [0, 0, 255]):
            # print("Oops")
            # return vertex
            return newNode
        newNode = tmp
...
def newConfigurasion(mymap, vertex, node, stepSize):
    direction = (node[0] - vertex[0], node[1] - vertex[1])

    return testReachability(mymap, vertex, direction, stepSize)
...
def rrtPlanning(mymap, goal, start, stepSize):
    ...
    for i in range(50000):
        ...
        newNode = newConfigurasion(mymap_, near, rand, stepSize)
        if newNode in parent:
            continue

        drift = parent[near][1] + 1
        parent[newNode] = (near, drift)
        #print("parent", parent)
        # Draw edge
        ...
        if (newNode[0] - goal[0])**2 + (newNode[1] - goal[1])**2 <= 5**2:
            print("Found the goal!")
            succ = newNode
            pred, drift = parent[succ]
            while pred:
                cv2.line(mymap, (succ[1], succ[0]), (pred[1], pred[0]), [255, 255, 0])
                cv2.circle(mymap, (succ[1], succ[0]), int(drift/10), [0, 255, 0])
                succ = pred
                pred, drift = parent[pred]
                # print(drift)
...
if __name__ == "__main__":
    ...
    # cv2.circle(mymap, (start), 5 [0, 0, 255])
